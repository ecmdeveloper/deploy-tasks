 ...
 *
 */
public class FolderTask extends ObjectStoreNestedTask {

	private String className;
	private String parentPath;
	private String name;
	private CustomObject folder2;
	...
	public void execute() throws BuildException {
		...
		folder2 = objectStoreAction.doQuery(CustomObject.class, getObjectStore(), query); // TODO make sure only on object matches?
		
		if ( folder2 == null) {
			newCustomObject = true;
			log("Creating new Custom Object");
			if ( id == null ) {
				folder2 = Factory.CustomObject.createInstance(getObjectStore(), className);
			} else {
				folder2 = Factory.CustomObject.createInstance(getObjectStore(), className, new Id(id) );
			}
		} else {
			log("Updating existing Custom Object");
		}
		
		updateObjectStoreObjectAction.execute(this, folder2);
		updateSecurityAction.execute(this, folder2);
		
		batch.add(folder2, null);
		...
		if ( newCustomObject && parentPath != null) {
			...
			ReferentialContainmentRelationship relationship = folder.file(folder2, AutoUniqueName.NOT_AUTO_UNIQUE, name, DefineSecurityParentage.DO_NOT_DEFINE_SECURITY_PARENTAGE);
